...
public class BaseSpotScorer extends ConfigScorer{

	private Logger logger=Logger.getLogger(getClass());
	//public static final Map<String,Integer> tagFrenquency=new HashMap<String,Integer>();
	
	public BaseSpotScorer(List<BaseSpot> allcitysights,List<BaseSpot> requiredsights, List<Tag> tags,
	...
	private void init(List<BaseSpot> allcitysights){
	...
	private void extract(List<BaseSpot> allcitysights,List<BaseSpot> requiredsights,List<Tag> tags,Double lat,Double lon){
		List<Tag> thistags;
		for(int i=0;i<allcitysights.size();i++){
			...
			if(allcitysights.get(i).getCount()!=null){
			data[i][0]=allcitysights.get(i).getCount();}
			else{
			data[i][0]=1;
			}
			data[i][1]=0;
			if(allcitysights.get(i).getRank()!=null){
			data[i][2]=allcitysights.get(i).getRank();}
			else {
				data[i][2]=1;
			}
	...
	public double[] score(List<BaseSpot> allcitysights){
